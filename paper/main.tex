...
\end{tabular}
\end{center}
\footnotesize{*Hard cases only: 48 targets missed by vanilla. Combined = vanilla on easy cases + expansion on hard cases.}
\end{table}
...
We show that simple candidate expansion significantly improves Hit@10 on the Sci-Reasoning benchmark, from 37.7\% to 58.4\%. This suggests that the bottleneck in scientific idea generation is not the quality of individual ideas but the diversity of the candidate set. We also find that innovation pattern predictability varies dramatically, with Representation Shift patterns being most predictable and Cross-Domain Synthesis patterns being least predictable.
